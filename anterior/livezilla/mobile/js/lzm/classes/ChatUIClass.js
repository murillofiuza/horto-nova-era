...
ChatUI.prototype.CreateListLine = function (_chat, _isBot) {
    ...
    if(lzm_chatDisplay.IsFullscreenMode())
    {
        lineHtml = '<tr class="allchats-line'+missedClass+'" id="allchats-line-' + _chat.i + '"' + onclickAction + oncontextmenuAction + ' style="cursor: pointer;">' + that.GetButtonField(_chat)+that.GetIconField(_chat,missed,closed,_isBot);
        for (i=0; i<LocalConfiguration.TableColumns.allchats.length; i++) {
